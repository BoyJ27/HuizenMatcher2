var Dilemma16Controller = function ( model, view ){
  ...
  view.volgendeButton.click(function(){
    ...
    //in database.
    $("table#case16 input:radio:checked").each(function () {
      checked = 1; // user made a choice
      house = this.value; //get the chosen house (house A, house B or no preference)

      //If house A is chosen
      if (house == "house A") {
        setDilemmaA(16, attributesHouseA, attributesHouseB); //Dilemma number,  array for A, array for B
      }
      //If house B is chosen
      else if (house == "house B") {
        setDilemmaB(16, attributesHouseA, attributesHouseB);
      }
      //If no preference
      else {
        setDilemmaNo(16, attributesHouseA, attributesHouseB);
      }
    });
