var Dilemma15Controller = function ( model, view ){
  ...
  view.volgendeButton.click(function(){
    ...
    //in database.
    $("table#case15 input:radio:checked").each(function () {
      checked = 1; // user made a choice
      house = this.value; //get the chosen house (house A, house B or no preference)

      //If house A is chosen
      if (house == "house A") {
        setDilemmaA(15, attributesHouseA, attributesHouseB); //Dilemma = 1, choice = 1 (house A), array for A, array for B
      }
      //If house B is chosen
      else if (house == "house B") {
        setDilemmaB(15, attributesHouseA, attributesHouseB);
      }
      //If no preference
      else {
        setDilemmaNo(15, attributesHouseA, attributesHouseB);
      }
    });
